...
library BorrowLogic {
  ...
  function executeBorrow(
    ...
    if (params.releaseUnderlying) {
      IMToken(reserveCache.aTokenAddress).transferUnderlyingTo(
        params.user,
        params.amount
      );
    }
  ...
  function executeRepay(
    ...
    // Update liquidity index and variable borrow index
    reserve.updateState(reserveCache);

    // Amount of stableDebtTokens and variableDebt tokens
    ...
    // Amount of stable or variable debt
    ...
    // Ensure amount to repay is less than debt
    ...
    // Set borrowing to false if total debt is repaid
    ...
    if (params.useATokens) {
      ...
      IMToken(reserveCache.aTokenAddress).handleRepayment(
        msg.sender,
        paybackAmount
      );
    }
