...
contract VariableDebtTokenFacet is IVariableDebtToken, OwnableInternal {
  /**
  ...
  function approveDelegation(address delegatee, uint256 amount) external {
    return LibVariableDebtToken.layout().approveDelegation(delegatee, amount);
  }
  ...
  function borrowAllowance(address fromUser, address toUser)
    ...
    returns (uint256)
  {
    return LibVariableDebtToken.layout()._borrowAllowances[fromUser][toUser];
  }
  ...
  function mint(
    ...
    return LibVariableDebtToken.layout().mint(user, onBehalfOf, amount, index);
  }
